...
public class PostController {
   ...
   // localhost:8080/api/posts?pageNo=1&pageSize=2              //if we give required as true = pageNo=1&pageSize=5  this is mandatory.
    ...
    @GetMapping("/{id}")
    public ResponseEntity<?> postById(@Valid @PathVariable("id") Long id,BindingResult result){
        if (result.hasErrors()){
    ...
    @PreAuthorize("hasRole('ADMIN')")
   @DeleteMapping ("/{id}")
    public ResponseEntity<?> deleteById(@Valid @PathVariable("id")Long id,BindingResult result){ //if we want to return message as a String we give ResponseEntity<String>.
       if (result.hasErrors()){
           return new ResponseEntity<>(result.getFieldError().getDefaultMessage(),HttpStatus.INTERNAL_SERVER_ERROR);
       }
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @PutMapping("/{id}")
    public ResponseEntity<?> UpdatePostById(@Valid @PathVariable("id") long id,@RequestBody PostDto postDto , BindingResult result){
        if (result.hasErrors()){
            return  new ResponseEntity<>(result.getFieldError().getDefaultMessage(),HttpStatus.INTERNAL_SERVER_ERROR);
        }
